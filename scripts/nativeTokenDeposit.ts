...
async function sendEthOrDepositERC20(
  ...
  if (nativeToken === ethers.constants.AddressZero) {
    ...
    // Call depositERC20 with INITIAL_FUND_AMOUNT_CREATOR tokens if nativeToken is not zero address.
    const amount = ethers.utils.parseEther(INITIAL_FUND_AMOUNT_CREATOR)
